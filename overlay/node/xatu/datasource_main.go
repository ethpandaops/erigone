...
func (s *Service) receiptsGenerator() *receipts.Generator {
	s.receiptsGenOnce.Do(func() {
		// erigon-main's NewGenerator additionally needs the datadir and a state
		// cache (used by ComputeBlockContext during re-execution). A fresh coherent
		// cache is fine here: unfed, it simply misses through to the tx, which is
		// correct (just no caching speed-up).
		s.receiptsGen = receipts.NewGenerator(
			s.dirs, s.blockReader, s.engine,
			kvcache.New(kvcache.DefaultCoherentConfig), receiptsEVMTimeout,
		)
	})
...
func (s *Service) BlockReceipts(ctx context.Context, number *big.Int) ([]execution.Receipt, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read commitment history flag: %w", err)
	}

	recs, err := s.receiptsGenerator().GetReceipts(ctx, s.chainConfigForExecution(ctx), tx, block,
		eth.ReceiptsOpts{CommitmentHistoryEnabled: commitmentHistory})
	if err != nil {
